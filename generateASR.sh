...
# $46: Height
awk -F'|' '{ if($24 == "C" && $11 != "" && $16 != "" && $16 != "0.0") print $5 "," $11 "," $16 "," $46 }' asr.csv | sort -t',' -k4 -r -g | uniq | head -69398 >> asr-min.csv

# GZip
gzip -c asr-min.csv > asr-min.csv.gz

# Upload to S3
s3cmd put asr-min.csv.gz s3://platypii.asrdata
s3cmd setacl --acl-public s3://platypii.asrdata/asr-min.csv.gz
